...
description: Act as the main creative agent for an end-to-end, multi-shot video production. Use when Codex or another agent host must turn a brief and references into character and scene design, script, storyboard, ComfyUI/H3 control drafts, stage-by-stage reviews, online final renders, deterministic HyperFrames assembly, and a traceable delivery; also use when resuming or repairing an existing production from the repository ledger.
---
...
3. Design the story, characters, locations, visual language, sound intent, continuity anchors, and acceptance criteria. Persist them as structured project records.
4. Produce a shot-by-shot storyboard. Give each shot one narrative purpose, one continuous action, explicit camera intent, ordered references, sound intent, and a measurable gate.
5. Choose the smallest control route for each shot. Invoke `generate-minimax-h3-shot` only when motion, camera, blocking, scene state, or identity continuity requires a control draft. Simple shots may go directly to final rendering.
6. Ask Runtime to execute the selected control Profile. Inspect the returned control asset with `review-video-candidate` at the relaxed `control-draft` gate. Accept, patch, or rerun only that shot.
7. Ask Runtime to submit each accepted Shot Intent plus any accepted control asset to the selected online final-render Provider. Treat Seedance, MiniMax cloud, LibTV-selected models, and Wan as replaceable execution routes, not as directors.
8. Inspect every final candidate with `review-video-candidate` at the strict `final-candidate` gate. Diagnose whether a defect belongs to planning, control, or final rendering, then retry only the responsible stage.
9. Freeze an `AcceptedShotManifest`. Assemble only accepted final shots. Use HyperFrames for deterministic ordering, trimming, titles, subtitles, graphics, transitions, and brand treatment; do not send the assembled master back through a generative model by default.
10. Run delivery QC and archive the complete lineage. Return the production ID, accepted-shot manifest, unresolved human-review items, cost summary, and final delivery location.
