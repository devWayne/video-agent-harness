...
| Technical quality | Duration, dimensions, frame rate, codec, audio sample rate/channels, A/V sync, corruption, black/frozen frames, hash lineage, and delivery compatibility |
...
For multi-segment motion graphics, inspect every boundary for endpoint agreement, duplicate/frozen motion, transition flashes, invented text/numbers, and semantic discontinuity. For an upscaled master, compare composition and duration against the picture lock; reject enhancement that changes timing or destroys brand-critical detail. For narration delivery, verify every Cue lands inside its declared picture window and manually check brand names, amounts, percentages, abbreviations, and final slogan pronunciation.
