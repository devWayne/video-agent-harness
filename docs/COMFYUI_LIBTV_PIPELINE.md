...
这条配方用于对动作、镜头轨迹、构图和节奏有明确要求的商业镜头：先用本地 ComfyUI/H3 生成控制视频，再把它作为 LibTV 在线模型的 Reference Video，生成材质、人物、灯光与时序质量更高的最终镜头。

它不是“先生成一次，再无意义地生成一次”。两个阶段具有不同目标：
...
- 尚未绑定并实测用户当前的 H3 API Workflow JSON；
