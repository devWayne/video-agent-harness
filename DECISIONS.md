...
- **决定：** 首版仅使用用户自己的阿里云百炼官方账号，优先跑通 Wan 3.0。
...
- **决定：** 百炼地域、Workspace ID、Wan 3.0 Model ID 和 API Key 均由用户在部署后台配置，不作为仓库初始化的前置条件。
...
- **底层依赖：** 优先评估 Pi Agent Core、HyperFrames 和 FFmpeg；Remotion 作为受许可证约束的可选 Renderer。
...
- **接入状态：** 已提交模型开通申请，控制台当前显示“申请中”；项目专用 API Key 已创建并写入本地忽略文件，待模型授权完成后执行真实连通测试。
